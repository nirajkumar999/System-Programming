...
read first line
 ...
 start = atoi(operand);
 // convert operand value from string to integer and assign
to start
 locctr = start;
 fprintf(fp4, "\t%s\t%s\t%s\n", label, opcode, operand);
 // write to output file (additional tab space as start will
not have any locctr)
 fscanf(fp1, "%s\t%s\t%s", label, opcode, operand);
 // read next line
 }
...
operand);
 ...
 // if opcode in input matches the one in optab,
increment locctr by 3
 ...
 // 5. Searching opcode for WORD, RESW, BYTE, RESB keywords
and updating locctr
 ...
 // convert operand to integer and multiply with 3
 }
